...
uint8_t AddressingMode_Execute(CPU *cpu, Instruction *inst){
	...
	switch (inst->opcode.addressingMode) {
		...
		case ZEX :
			...
			inst->dataMem = mapper->get(mapper->mapperData, AS_CPU, address);
			break;
		...
		case INX :
			address = (0x00FF) & (uint16_t)(inst->opcodeArg[0] + cpu->X);
			uint8_t lWeight = *(mapper->get(mapper->mapperData, AS_CPU, address));
			uint8_t hWeight = *(mapper->get(mapper->mapperData, AS_CPU, address + 1));
